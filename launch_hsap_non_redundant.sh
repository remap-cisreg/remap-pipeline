...
snakemake   --use-singularity --singularity-args "-B /scratch/bballester:/scratch/bballester" --snakefile /scratch/bballester/thaliana/remap-thaliana-tf/2.scripts/snakefiles_workflow/Snakefile_remap_non_redundant.py --cores 200  --printshellcmds --cluster-config 2.scripts/cluster_configuration/cluster_singularity_slurm_hsap_remap_non_redundant.json --cluster "sbatch  -A {cluster.project-name}  --job-name {cluster.job-name} -p {cluster.partition}  --ntasks {cluster.ntasks} --cpus-per-task={cluster.thread}  -o {cluster.stdout} -e {cluster.stderr}  --time {cluster.time}  --mem-per-cpu={cluster.memory} " --keep-going --configfile 2.scripts/snakemake_configuration/Snakefile_config_hsap_remap_non_redundant.json --rerun-incomplete --resources res=200

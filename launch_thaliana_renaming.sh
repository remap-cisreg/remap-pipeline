...
snakemake   --use-singularity --singularity-args "-B /scratch/bballester:/scratch/bballester" --snakefile 2.scripts/snakefiles_workflow/Snakefile_remap_renaming.py --cores 150  --printshellcmds --cluster-config 2.scripts/cluster_configuration/cluster_singularity_slurm_thaliana_remap_renaming.json --cluster "sbatch  -A {cluster.project-name}  --job-name {cluster.job-name} -p {cluster.partition}  --ntasks {cluster.ntasks} --cpus-per-task={cluster.thread}  -o {cluster.stdout} -e {cluster.stderr}  --time {cluster.time}  --mem-per-cpu={cluster.memory} " --keep-going --configfile 2.scripts/snakemake_configuration/Snakefile_config_thaliana_remap_renaming.json --rerun-incomplete --resources res=150

...
def numbering_replicat( list_remap_sampleID, list_ena_line,dico_GSM_to_SRX):
	...
	for remap_GSM in list_remap_sampleID:
		...
		for ena_line in list_ena_line:
			if remap_GSM in ena_line:
				...
				nb_rep_tech += 1
			elif remap_GSM in dico_GSM_to_SRX.keys():
				if dico_GSM_to_SRX[remap_GSM] in ena_line:
					list_sample_name.append( [ str( nb_rep_bio), str( nb_rep_tech)] + ena_line)
		...
		if final_link:
			...
			if final_library == 'PAIRED':
				...
				if len( final_link.split( ";")) > 2:
					...
					for i in range( len( list_split_finale_link)):
						...
						# else don't add link in list
							...
							list_temp_final_link.append( list_split_finale_link[ i])
							list_error.append( current_error)
							sys.stderr.write( current_error + "\n")
				...
				if len( final_link.split( ";")) == 1:
					...
					list_temp_final_link.append( final_link)
					list_error.append( current_error)
					sys.stderr.write( current_error + "\n")
...
# 	url_experiment_srt = "https://www.ebi.ac.uk/ena/data/warehouse/filereport?accession="
	...
	path_file_remap_metadata="/home/souville/Documents/Projets/Remap/sample_debug/EXAMPLE_hsap_TF.tsv"
	col_ID_serie = 0
	col_TF = 1
	col_cell = 2
	col_SRP = 5
	...
	for line in file_remap_metadata:
		...
		if  not line_error:
			...
			for line_ena_resp in iter_line_ena_resp:
				...
				try:
					# add only relevant line
                                        ...
                                        cond1 =  list_info_ena[ 4] in list_chip_GSM
                                        cond2 =  list_info_ena[ 4] in list_control_GSM
                                        cond3 =  maybe_gsm in list_chip_GSM
                                        cond4 =  maybe_gsm in list_control_GSM
                                        #print(list_control_GSM)
                                        #print(list_chip_GSM)
                                        # if cond1 == False and cond2 == False and cond3 == False and cond4 == False:
                                        #         for GSM in list_chip_GSM:
                                        #                 GSM_id =  str(GSM)
                                        #                 result = subprocess.run(['pysradb','gsm-to-srx',GSM_id], stdout=subprocess.PIPE)
                                        #                 out = result.stdout.decode("utf-8")
                                        #                 #print(out)
                                        #                 for elem in out.split(' '):
                                        #                         if elem.startswith('SRX'):
                                        #                                 SRX = elem
                                        #                                 list_chip_SRX.append(SRX)
                                        #                                 dico_chip_SRX[GSM] = SRX
                                        #                                 dico_chip_SRX[SRX] = GSM
                                        #         #if len(list_control_GSM) > 0:
                                        #         #        print(len(list_control_GSM))
                                        #         for GSM in list_control_GSM:
                                        #                 GSM_id = str(GSM)
                                        #                 if GSM_id != '':
                                        #                         result = subprocess.run(['pysradb','gsm-to-srx',GSM_id], stdout=subprocess.PIPE)
                                        ...
                                        #                                 if elem.startswith('SRX'):
                                        #                                         SRX = elem
                                        #                                         list_control_SRX.append(SRX)
                                        #                                         dico_control_SRX[GSM] = SRX
                                        #                                         dico_control_SRX[SRX] = GSM
                                        #print(list_chip_SRX)
                                        if list_info_ena[ 2] in list_chip_SRX:
                                                #print(list_info_ena[ 2])
                                                list_line_chip_ena.append( list_info_ena)
                                                list_found_GSM.append(dico_chip_SRX[list_info_ena[ 2]])
                                        if list_info_ena[ 2] in list_control_SRX:
                                                list_line_chip_ena.append( list_info_ena)
                                                list_found_GSM.append(dico_control_SRX[list_info_ena[ 2]])
			...
			if check_GSM_ena:
				...
				#print(list_line_chip_ena)
				# Only process info if GEO info are found on ENA
				if list_line_chip_ena:

					# Creating replicat number for each file
					list_chip_exp_ena = numbering_replicat( list_chip_GSM, list_line_chip_ena,dico_chip_SRX)
					list_control_exp_ena = numbering_replicat( list_control_GSM, list_line_control_ena,dico_control_SRX)
					...
					error_nb_link = False
					for sample_error in list_all_sample_error:
						try:
							if "has too many donwload links:" in sample_error[ 0]:
								error_nb_link = True
						except IndexError:
							pass
					...
					if error_in_exp and not error_nb_link:
					...
					# If there is no error in the experiment
						...
						if error_nb_link:
							path_outdir_experiment = os.path.join( path_outdir_error)
							path_outfile_experiment = os.path.join( path_outdir_experiment, proposed_name_experiment + "_too_many_download_links.tab")
							outfile_experiment = open( path_outfile_experiment, 'w')
						else:
							# Writting results

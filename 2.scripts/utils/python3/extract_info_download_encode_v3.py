...
if __name__ == "__main__":
	...
	for line in file_encode_metadata:
		...
		biological_replicat_name = experiment_name + "." + biological_replicate
	...
	for experiment_name in final_dict_experiment:
		...
		for biological_replicat in final_dict_experiment[ experiment_name][ 'chip']:
			...
			if  len( final_dict_experiment[ experiment_name][ 'chip'][ biological_replicat][ 'single-end']):
				for i in range( len( final_dict_experiment[ experiment_name][ 'chip'][ biological_replicat][ 'single-end'])):
					list_final_info = [ biological_replicat + "-" + str( i + 1)] + \
			...
			if  len( final_dict_experiment[ experiment_name][ 'chip'][ biological_replicat][ 'paired-end']):
				...
				for technical_replicat in final_dict_experiment[ experiment_name][ 'chip'][ biological_replicat][ 'paired-end']:
					...
					elif  len( temp_dict_control[ biological_replicat][ 'paired-end'][ technical_replicat]) <2:
		...
		# print( final_dict_experiment[ experiment_name][ 'control'])
		...
		temp_dict_control = {}
		control_name = ".".join( [
									final_dict_experiment[ experiment_name][ 'experiment_accession'],
									"INPUT",
									final_dict_experiment[ experiment_name][ 'biosample_name']
								 ]
								)
		# For all control of this experiment get info
		for url_controlled_by in final_dict_experiment[ experiment_name][ 'control']:

			biological_replicat_name = ".".join( [control_name, str( dict_control_info[ url_controlled_by][ 'biological_replicat'])])

			# If this biological replicat was not already in the dictionary
			if biological_replicat_name not in temp_dict_control:
				temp_dict_control[ biological_replicat_name] = {}
				temp_dict_control[ biological_replicat_name][ 'paired-end'] = {}
				temp_dict_control[ biological_replicat_name][ 'single-end'] = []

			run_type = dict_control_info[ url_controlled_by][ 'run_type']
			if run_type == 'single-ended':
				list_info = [ 	dict_control_info[ url_controlled_by][ 'url_control_encode'],
								'SINGLE',
								"1",
				...
				temp_dict_control[ biological_replicat_name][ 'single-end'].append( list_info)
				...
				pair_with = dict_control_info[ url_controlled_by][ 'pair_with']

				# Check if buddy is already in the dictionary
				if pair_with in temp_dict_control[ biological_replicat_name][ 'paired-end']:
					temp_dict_control[ biological_replicat_name][ 'paired-end'][ pair_with].append( list_info)
				# If buddy is not in the dictionary add a key with file name
				else:
					temp_dict_control[ biological_replicat_name][ 'paired-end'][ url_controlled_by] = [ list_info]


		# print( temp_dict_control)


		# Getting all info for control
		for biological_replicat in temp_dict_control:
			# dealing with single en
			if  len( temp_dict_control[ biological_replicat][ 'single-end']):
				for i in range( len( temp_dict_control[ biological_replicat][ 'single-end'])):
					list_final_info = [ biological_replicat + "-" + str( i + 1)] + \
					 					temp_dict_control[ biological_replicat][ 'single-end'][ i]
			...
			# dealing with paired end
			if  len( temp_dict_control[ biological_replicat][ 'paired-end']):
				nb_technical_replicat = 0
				# for all technical replicat
				for technical_replicat in temp_dict_control[ biological_replicat][ 'paired-end']:
					nb_technical_replicat += 1
					if len( temp_dict_control[ biological_replicat][ 'paired-end'][ technical_replicat]) == 2:

						paired_info = join_paired_info( temp_dict_control[ biological_replicat][ 'paired-end'][ technical_replicat], 0)
						paired_url_download =  join_paired_info( temp_dict_control[ biological_replicat][ 'paired-end'][ technical_replicat], 3)
						paired_md5 = join_paired_info( temp_dict_control[ biological_replicat][ 'paired-end'][ technical_replicat], 4)


						list_final_info =  	[ biological_replicat + "-" + str( nb_technical_replicat),
											paired_info] + \
											temp_dict_control[ biological_replicat][ 'paired-end'][ technical_replicat][ 0][ 1:3] + \
											[ paired_url_download,
											paired_md5]
						list_line.append( list_final_info)
						# file_experiment.write( "\t".join( list_final_info) + "\n")
						...
						list_final_info =  	[ biological_replicat + "-" + str( nb_technical_replicat),
											paired_info]
						list_line.append( list_final_info)
						# print( dict_control_info[ url_control][ 'warning'])
					else:
						error_experiment = True
						sys.stderr.write( biological_replicat + "-" + str( nb_technical_replicat) + " paired end replicat has too many files\n")
						list_final_info =  	[ biological_replicat + "-" + str( nb_technical_replicat),
		...
		if error_experiment:
			dir_path_file_experiment = os.path.join( path_outdir_error, experiment_name)
			dir_path_file_experiment_log = os.path.join( path_outdir_error, experiment_name, "log")
		else:
			dir_path_file_experiment = os.path.join( path_outdir, experiment_name)
			dir_path_file_experiment_log = os.path.join( path_outdir, experiment_name, "log")

		# creating output dir
		if not os.path.exists( dir_path_file_experiment):
			os.makedirs( dir_path_file_experiment)

		if not os.path.exists( dir_path_file_experiment_log):
			os.makedirs( dir_path_file_experiment_log)

...
if __name__ == "__main__":
	...
	for line in file_encode_metadata:
		...
		experiment_target = split_line[ column_experiment_target].split('-')[0]
	...
	for url_control in dict_control_file:
		...
		url_control_encode = 'https://www.encodeproject.org/'+url_control.strip().split('@')[0]
		...
		# Get all Info
		if len(dict_control_file_raw['biological_replicates']) > 0:
			dict_control_info[ url_control][ 'biological_replicat'] = dict_control_file_raw['biological_replicates'][-1]
		else:
			dict_control_info[ url_control][ 'biological_replicat'] = ''

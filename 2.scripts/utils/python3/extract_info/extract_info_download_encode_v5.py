...
if __name__ == "__main__":
	...
	column_audit_warning = 52
	column_audit_internal_action = 54
	column_audit_not_compliant = 53
	...
	for line in file_encode_metadata:
		...
		if controlled_by:
                        ...
                        for url_controlled_by in list_controlled_by:
                                ...
                                url_control_encode = "https://www.encodeproject.org" + clean_url_controlled_by.strip()
                                url_control_encode_json = url_control_encode + "?format=json"
                                try:
                                        url =  urllib.request.urlopen( url_control_encode_json)
                                        if clean_url_controlled_by not in dict_control_file:
                                                dict_control_file[ clean_url_controlled_by] = {}
                                except: 
                                        list_ctrl_innac.append(clean_url_controlled_by)
                                        error_url = 1
		...
		if error_url == 1:
                        ...
                        path_file_experiment = os.path.join( dir_path_file_experiment, experiment_name + "_url_err.tab")
                        file_experiment = open( path_file_experiment, 'w')
                        file_experiment.write('url inaccessible  des controles : ' + ';'.join(list_ctrl_innac))
                        file_experiment.close()
                        del final_dict_experiment[ experiment_name]

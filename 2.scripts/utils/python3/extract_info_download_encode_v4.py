...
if __name__ == "__main__":
	...
	column_experiment_target = 21
	column_biological_replicate = 33
	column_technical_replicate = 34
	column_run_type = 37
	column_pair_with = 39
	column_md5 = 44
	column_url_download = 46
	column_controlled_by = 49
	column_file_status = 50
	column_audit_warning = 54
	column_audit_internal_action = 56
	column_audit_not_compliant = 55
	...
	for url_control in dict_control_file:
		...
		try:
			...
			error_url = 1
			dict_control_info[ url_control][ 'error_url'] = 1
			continue
		else:
                        dict_control_info[ url_control][ 'error_url'] = 0
		#print( "ok")
		# print( )

		# Get all Info
		#print(dict_control_file_raw)
		...
		dict_control_info[ url_control][ 'run_type'] = 'single-ended'
	...
	for url_control in dict_control_info:
		...
		if dict_control_info[ url_control][ 'run_type'] == 'paired-ended':
			if error_url == 1:
                        	dict_paired_control[ 'PAIRED'][ url_control][ 'error_url'] = 1
                        	continue
			...
			if error_url == 1:
                                dict_paired_control[ 'SINGLE'][ url_control][ 'error_url'] = 1
                                continue
	...
	for experiment_name in final_dict_experiment:
		...
		if dict_control_info[url_control][ 'error_url'] == 1:
		...
		for url_controlled_by in list_new_control:
			...
			if dict_control_info[url_controlled_by][ 'error_url'] == 1:
                        	print("url error for experiment "+experiment_name)
                        	...
                        	# writting in file
                        	path_file_experiment = os.path.join( dir_path_file_experiment, experiment_name + "_url_not_found.tab")
                        	file_experiment = open( path_file_experiment, 'w')

                        	file_experiment.write( "url_not_found for experiment : "+experiment_name)

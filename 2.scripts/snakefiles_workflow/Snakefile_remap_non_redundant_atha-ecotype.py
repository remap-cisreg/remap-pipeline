...
include: os.path.join(BASE_DIR, RULE_DIR, "tf_allPeaks_ecotype.rules")
include: os.path.join(BASE_DIR, RULE_DIR, "tf_nrPeaks_median_size_peak_ecotype.rules")
include: os.path.join(BASE_DIR, RULE_DIR, "tf_nrPeaks_merge_ecotype.rules")
include: os.path.join(BASE_DIR, RULE_DIR, "tf_nrPeaks_ecotype.rules")

include: os.path.join(BASE_DIR, RULE_DIR, "cell_allPeaks_ecotype.rules")
...
for line in file_peaks:
    ...
    cell = split_info_column[ -1].split( "_")[0] + '_' + split_info_column[ -1].split( "_")[1]
    #print(cell)
    set_cell.add( cell)
...
#                         Workflow                               #
#================================================================#
NR_NAME = os.path.join(PREFIX + "_nr-ecotype_" + SUFFIX + ".bed")

rule all:
    input:
            expand( os.path.join( BED_DIR, "TF_ECOTYPE", "{tf}" , PREFIX + "_{tf}_nr_" + SUFFIX + ".bed"), tf = set_tf),
            expand( os.path.join( BED_DIR, "TF_ECOTYPE", "{tf}" , PREFIX + "_{tf}_all_" + SUFFIX + ".bed"), tf = set_tf),
            #expand( os.path.join( BED_DIR, "ECOTYPE_BIOTYPE","{cell}" , PREFIX + "_{cell}_nr_" + SUFFIX + ".bed"), cell = set_cell),
            expand( os.path.join( BED_DIR, "ECOTYPE_BIOTYPE","{cell}" , PREFIX + "_{cell}_all_" + SUFFIX + ".bed"), cell = set_cell),
...
rule merge_all_nr_bed:
    input:
            expand( os.path.join( BED_DIR, "TF_ECOTYPE", "{tf}" , PREFIX + "_{tf}_nr_" + SUFFIX + ".bed"), tf = set_tf)
    output:
            os.path.join(PREFIX + "_nr-ecotype_" + SUFFIX + ".bed")	
    params:
            unsort_bed = temp(os.path.join(PREFIX + "_nr-ecotype_" + SUFFIX + "_unsort.bed"))
